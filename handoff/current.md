...
## F2 — Migração dos componentes 🟠 *(4/9 concluídos, 2026-07-03, branch `v2` do dev-hard)*
...
- Guard `check-camadas.sh`: `PENDING_V1` em **12 arquivos** (era 17 no início da F1).
- Docs espelhados em `ozi-ui-docs/dev/{behaviors/ozi-toggle,modules/ozi-loaddata,components/ozi-select}/`; `ozi-validate`/`ozi-loaddata`/`ozi-select` também documentam no próprio `dev-hard` (CHANGELOG/README ao lado do JS).

## Próximo passo recomendado

**F2 #5 — migrar `ozi-autocomplete`** (724 linhas, 24 jQuery) na branch `v2`:
reaproveita boa parte dos padrões já estabelecidos no `ozi-select` (adapter `nativeElement:true`, delegação consolidada, `emit()` com `source`). Checklist no projeto consolidado §F2 (sem jQuery; emissão só via `OZI.helpers.emit`; init idempotente + destroy; classes `ozi-*`; i18n; espelhar doc; shim de compat em `integrations/` se necessário; página de aceite). Ao concluir: remover do `PENDING_V1` do guard. Depois: check (#6, pequeno, recém-refatorado).
